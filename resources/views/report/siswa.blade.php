...
<div class="card mb-3">
    <div class="card-header bg-light d-flex justify-content-between flex-wrap">
       ...
       <div id="loader" class="loader"></div>
        <div class="d-flex align-items-center flex-wrap">
            <div class="">
                <select id="kelasFilter" class="form-select select2" style="min-width: 200px;">
                    <option value="" disabled selected>Pilih Kelas...</option>
                    @foreach ($kelas as $key)
                        <option value="{{ $key->id }}">
                            {{ $key->nama_kelas .' '.$key->jurusanKelas->nama_jurusan .' '.$key->sub_kelas }}
                        </option>
                    @endforeach
                </select>
            </div>
    ...
    <div class="card-body my-0">
        <iframe id="previewFrame" src="/export/students?kelas={{ $kelas->first()->id ?? 1 }}" width="100%" height="600px" frameborder="0"></iframe>
    </div>
</div>
...
<script>
    ...
    // Fungsi untuk mengganti URL pada iframe ketika filter kelas berubah
    function updateIframe() {
        ...
        // Ubah sumber iframe
        iframe.src = `/export/students?kelas=${selectedKelas}`;

        // Ketika iframe selesai dimuat, sembunyikan loader
        iframe.onload = () => {
            document.getElementById('loader').style.display = 'none';
        };
    }

    // Deteksi perubahan pada select2
